...
async function AddComplaint(req, res) {
    try {
        ...
        const { artistId, bookingId, eventName, artistName, subject, message } = req.body;

        if (!artistId || !bookingId || !eventName || !artistName || !subject || !message) {
        ...
        const userId = await session.session._id;
