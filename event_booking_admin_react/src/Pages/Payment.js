...
function Payment() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://localhost:8000/admin/getPayment");
  ...
  const handleGenerateReport = () => {
    ...
    if (fromDate || toDate) {
      filteredData = formattedData.filter((row) => {
        ...
        if (fromDate) {
          ...
          fromDateObj = new Date(`${fromDateParts[2]}-${fromDateParts[1]}-${fromDateParts[0]}`);
          fromDateObj.setHours(0, 0, 0, 0); // Reset time to midnight
        }

        if (toDate) {
          const toDateParts = toDate.split('/');
          toDateObj = new Date(`${toDateParts[2]}-${toDateParts[1]}-${toDateParts[0]}`);
    ...
    const totalAmount = filteredData.reduce((total, row) => total + parseFloat(row.payments), 0);

    const doc = new jsPDF();
    doc.autoTable({
      head: [['Index', 'Payment ID', 'Booking ID', 'Payment Amount', 'Status', 'Date & Time']],
      body: filteredData.map((row, index) => [index + 1, row.paymentId, row.bookingId, row.payments, row.status, `${row.date} ${row.time}`]),
    });
